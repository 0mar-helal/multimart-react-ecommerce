  ...
  reducers: {
    ...
    decreaseQty: (state, action) => {
      ...
      if (productExit.qty === 1) {
        ...
        state.cartList = state.cartList.map((item) => (item.id === productExit.id ? { ...productExit, qty: productExit.qty - 1 } : item));
      }
    },
    ...
    // this function will clear the cart after the stripe's payment is successful
    clearCart: (state) => {
      state.cartList = [];
    },
  },
